...
public class DriveDistance2 extends CommandBase {
  ...
  public DriveDistance2(Drive subsystem, double distance) {
  ...
  public DriveDistance2(Drive subsystem, double distance, double error) {
    m_subsystem = subsystem;
    addRequirements(subsystem);
    target_distance = distance;
    if(error < 1.0){
  ...
  public boolean isFinished() {
    // could also add a failsafe to stop after 10 seconds under all circumstances
    current_rotations =  (m_subsystem.getLeftRotations() + m_subsystem.getRightRotations())/2.0;//takes average, probably unnecssary
    ...
    if(current_rotations > (target_rotations - error_rotations) && current_rotations < (target_rotations + error_rotations)){
      return true;
    }
    return false;
  }
